...
#include "helpers.h"
#include "bme_manager.h"
#include "ota_manager.h" // ← вместо <ArduinoOTA.h>
...
// --- ЗАДАЧА ДЛЯ ПОГОДЫ с гибким интервалом ---
void weatherTask(void *pvParameters)
{
  static const int WEATHER_STARTUP_RETRY_SEC = 5;
  ...
  while (1)
  {
    if (appState.wifiConnected)
    {
      ...
      // Если погода ещё ни разу не загружалась — повторяем быстро,
      // иначе используем настраиваемый интервал (60..86400 сек)
      int timeoutSec = (weatherData.lastUpdate == 0)
                           ? WEATHER_STARTUP_RETRY_SEC
                           : constrain(appState.weatherTimeoutSec, 60, 86400);
      vTaskDelay(pdMS_TO_TICKS((uint32_t)timeoutSec * 1000UL));
...
void loadConfig()
{
  ...
  // Гибкий интервал погоды (значение по умолчанию берётся из appState)
  appState.weatherTimeoutSec = prefs.getUInt(
      "weatherTmo",
      prefs.getUInt("weatherTimeoutSec", appState.weatherTimeoutSec));
...
// allowRestart=true — при старте (перезагружается если не подключился)
// allowRestart=false — при переподключении из loop (просто возвращает false)
bool connectWiFi(bool allowRestart = true)
{
  if (appState.isFirstBoot || appState.wifiSSID.length() == 0)
  {
    Serial.println("[WiFi] AP mode");
  ...
  if (WiFi.status() == WL_CONNECTED)
  {
    ...
    while (!WiFi.hostByName("google.com", ip))
    {
      ...
      if (tries >= 10)
      {
        ...
        Serial.println("[WiFi] DNS failed! Will retry later.");
        appState.isDataLoading = false;
        appState.wifiConnected = false;
        updateConnectionStatus();
        return false;
...
void setup()
{
  ...
  // --- Тяжёлые инициализации ---
  initDeviceControl();
  extern void initButton();
  initButton();
  initLeds();
  setLedColor(appState.ledColor);
  setLedMode(appState.defaultLedMode);
  initBusSchedule();
  initWeather();
  initOtaManager(); // ← вместо setupOTA()
...
void loop()
{
  handleButtonPress();
  updateWebSocketClient();
  updateOtaManager(); // ← вместо ArduinoOTA.handle()

  // Детектируем потерю WiFi
  if (appState.wifiConnected && WiFi.status() != WL_CONNECTED)
  {
    Serial.println("[WiFi] Connection lost");
    appState.wifiConnected = false;
    updateConnectionStatus();
  ...
  // Автопереподключение при потере WiFi — без перезагрузки
  if (!appState.wifiConnected && !appState.isFirstBoot && appState.wifiSSID.length() > 0)
  {
